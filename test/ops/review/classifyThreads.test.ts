...
// (src/ops/review/classifyThreads.ts + classify.config.ts).
//
// Pinned here:
//   1. The FULL decision table, one test per row (rows 1–12 of the module
//      doc): threads (resolved / responder-authored / bot-skip / outdated /
//      responder-last-word / fallback), reviews (bot-skip /
//      already-answered / fallback), top-level comments (responder-authored
//      / bot-skip / fallback).
//   2. Row-order precedence: resolved beats outdated (1 > 4),
//      responder-authored beats bot-skip and outdated (2 > 3, 2 > 4), and
//      the last-word row only sees threads rows 1–4 did not claim.
//   3. Null handling: null/unparseable timestamps fall back per
//      config.treatNullCreatedAtAs (both values), a null authorLogin is
//      never the responder (external, fails toward actionable), and a null
//      responder matches nobody.
//   4. nowMs is the ONLY clock: moving the injected nowMs flips a verdict.
//   5. Every default skipPattern fires on its phrasing class and none fires
//      on a normal review.
//   6. A single state exercises all five frozen verdicts.
//
...
// ---------------------------------------------------------------------------
// The decision table — rows 1–12, one test each
// ---------------------------------------------------------------------------
...
const ROW_CASES: RowCase[] = [
  ...
  {
    name: 'row 7 — a review whose body is a bot skip notice → skip (bot_skip_notice)',
    state: baseState({ reviews: [review({ id: 'R1', body: 'Review failed: internal error' })] }),
    expected: [{ kind: 'review', id: 'R1', verdict: 'skip', path: null, reason: 'bot_skip_notice' }],
  },
  {
    name: 'row 8 — a responder reply postdates the review → skip (review_already_answered)',
    state: baseState({
      reviews: [review({ id: 'R2', body: 'Please change X', submittedAt: T1 })],
      restReviewComments: [restComment({ id: 501, authorLogin: 'pr-author', createdAt: T2 })],
    }),
    expected: [
      { kind: 'review', id: 'R2', verdict: 'skip', path: null, reason: 'review_already_answered' },
    ],
  },
  {
    name: 'row 9 — a plain review summary → actionable (review_summary_needs_response)',
    state: baseState({ reviews: [review({ id: 'R3', body: 'Please add a test' })] }),
    expected: [
      { kind: 'review', id: 'R3', verdict: 'actionable', path: null, reason: 'review_summary_needs_response' },
    ],
  },
  {
    name: 'row 10 — a conversation comment the responder authored → skip (responder_authored)',
    state: baseState({
  ...
  {
    name: 'row 11 — a conversation comment matching a skip pattern → skip (bot_skip_notice)',
    state: baseState({
  ...
  {
    name: 'row 12 — a plain top-level comment → actionable (top_level_summary)',
    state: baseState({
...
describe('classifyThreads decision table', () => {
  test.each(ROW_CASES)('$name', ({ state, expected }) => {
    expect(classifyThreads(state, NOW)).toEqual(expected);
  });
});
...
describe('row-order precedence', () => {
  test('row 1 beats row 4 — a resolved outdated thread → resolved, not blocked', () => {
    const items = classifyThreads(
      baseState({ threads: [thread({ id: 'TP1', isResolved: true, isOutdated: true })] }),
      NOW,
    );
...
// ---------------------------------------------------------------------------
// Null handling — timestamps and authorLogins
// ---------------------------------------------------------------------------
  ...
  test("default 'nowMs': null review submittedAt is NOW, so a past responder reply does NOT answer it → actionable", () => {
    const items = classifyThreads(
      baseState({
        ...
        restReviewComments: [restComment({ id: 511, authorLogin: 'pr-author', createdAt: T1 })],
      }),
      NOW,
    );
    expect(items[0]?.verdict).toBe('actionable');
...
describe('null authorLogin is never the responder (fails toward actionable)', () => {
  test('a null-author thread is external — actionable, not skipped', () => {
    const items = classifyThreads(baseState({ threads: [thread({ id: 'TA1', authorLogin: null })] }), NOW);
    expect(items).toEqual([
...
// ---------------------------------------------------------------------------
// nowMs is the only clock
// ---------------------------------------------------------------------------

describe('injected nowMs drives the verdicts', () => {
  test('the same state flips skip → actionable as nowMs moves before the review', () => {
    ...
    // nowMs after the review: the null-createdAt reply falls back to nowMs
    // and postdates the review → already answered.
    const after = classifyThreads(state, Date.parse(T3));
    expect(after[0]?.verdict).toBe('skip');
    expect(after[0]?.reason).toBe('review_already_answered');
    // nowMs before the review: the same fallback lands before submittedAt →
    // still outstanding.
    const before = classifyThreads(state, Date.parse(T1));
    expect(before[0]?.verdict).toBe('actionable');
    expect(before[0]?.reason).toBe('review_summary_needs_response');
  });
});

// ---------------------------------------------------------------------------
// skipPatterns — every default pattern fires; normal reviews never match
// ---------------------------------------------------------------------------

describe('default skipPatterns', () => {
  test.each([
    ['CodeRabbit', 'CodeRabbit skipped this PR because the diff was empty'],
    ['review failed', 'Review failed: the tool errored out before finishing'],
    ['skipping review', 'Skipping review for this draft PR'],
    ['not reviewing', 'Not reviewing until the CI settles'],
  ])('pattern %s fires → bot_skip_notice', (_label, body) => {
    const items = classifyThreads(
      baseState({ restIssueComments: [restComment({ id: 621, body })] }),
      NOW,
    );
    expect(items).toEqual([{ kind: 'comment', id: '621', verdict: 'skip', path: null, reason: 'bot_skip_notice' }]);
  });
