...
describe('eval-axes-select module: --only selection and the credential gate (pure, dist-free)', () => {
  ...
  it('selectCells throws the usage message on a missing value and on a no-match', () => {
    ...
    expect(() => selectCells(CELLS, ['node', 'x', '--only', 'bogus'])).toThrow("no cell matches 'bogus'");
  ...
  it('requiredKeys: a zai-only selection with ZAI_API_KEY set needs nothing', () => {
    ...
    expect(requiredKeys(selected, EVAL_AXES_PROVIDER_KEYS, { ZAI_API_KEY: 'x' })).toEqual({ missing: [], unmapped: null });
  });
...
describe.skipIf(!existsSync(join(ROOT, 'dist', 'index.js')))('demo-eval-axes: the #30 selective credential gate (spawn e2e)', () => {
