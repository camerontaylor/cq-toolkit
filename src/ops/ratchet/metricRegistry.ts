...
//
// RENAMED from `registry.ts` at phase-4 T4.2: the family's `registry.ts`
// must be the OP registry (the central scanner's `<family>/registry.js`
// convention), so this interim internal registry moved to its own module.
// It is still re-exported through the family barrel (`./index.js`) and every
// adapter keeps importing it by id.
//
...
// always INJECTED by the caller via MetricSource (lane C's CheckRunner is
// now wired as the default in `./sources.js`, bound at importer time); this
// module must never import from src/gates or src/harness, so baseline
// capture stays computable without a live toolchain.
//
