...
- One subcommand per SHIPPED PLAN, generated from the plan registry
  ...
  A plan subcommand takes run-plan's governed-run flags MINUS --plan
  (RunPlanOptionsSchema) and runs the registry entry's discoverable floor
  plan through the same governed composition (runPlan + withBudgetStop);
