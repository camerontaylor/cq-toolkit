...
| `{{COMMANDS...}}`         | required-check.md                    | HAND-REPLACED slot (see above): the variadic ordered run-steps of the static job. The template carries this repo's six literal steps — static gate, format check, test, Knip, build, generated-op-docs drift — and an adopter replaces them by hand with their own commands; no placeholder text is substituted.                                                                                                                                                                                                                                                    |
| `{{SELFHOST_TOKEN}}`      | self-host workflows                  | NAME of the repository secret holding the automation token the entries ride (`GH_TOKEN` at run time; repo read + PR read/comment/merge + Contents write scope — Contents write because the conflict path pushes resolved branches) — instantiated files reference it as `${{ secrets.<name> }}`; a name, never a value                                                                                                                                                                                                                                              |
| `{{SELFHOST_DRIVER_KEY}}` | self-host workflows                  | NAME of the repository secret holding the model-provider API key the fix/conflict workers' driver reads (`ZAI_API_KEY` at run time) — instantiated files reference it as `${{ secrets.<name> }}`; a name, never a value                                                                                                                                                                                                                                                                                                                                             |
