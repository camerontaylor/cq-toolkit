...
node -e "import('./dist/plans/registry.js').then(async (m) => \
  (await m.listPlans()).forEach((e) => console.log(e.name)))" | sort
node dist/cli.js --help
```

Every op is reachable at a subcommand whose name is exactly its registry
entry name (`cq <op-name> …`); `run-plan` is the built-in plan-file
subcommand (governed plan composition), and every shipped plan is ALSO a
subcommand generated from the plan registry (`cq sweep`, `cq test-fix`,
`cq review-loop`, `cq merge-prs`, `cq analyze` — T4.3). A family's entries
live in `src/ops/<family>/registry.ts` — the central scanner
...
| Shipped plans (`sweep`, `test-fix`, `review-loop`, `merge-prs`, `analyze`) + CLI                                                | `src/plans/*.ts` (plan registry) + `src/cli/**`                    | _plan registry (`src/plans/registry.ts`), not op entries_                                                                                                          | one subcommand per plan — `cq sweep`, `cq test-fix`, `cq review-loop`, `cq merge-prs`, `cq analyze` (generated from the plan registry, T4.3); `run-plan --plan=<path>` runs an arbitrary plan JSON |
...
contracts. The plan-subcommand surface is generated from the plan registry
(`src/registry/plans.ts` → `src/cli/plans.ts`): `test/cli/plans.smoke.test.ts`
runs every discovered plan through the BUILT CLI and fails when a new plan
lands without a smoke row.
