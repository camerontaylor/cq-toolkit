...
- `fetchReviewState` preserves review actor type, association, and `commit.oid`,
  ...
  configuration.
- `classifyPrs` folds only verdict-carrying opinions per actor under the
  resolved trust policy, requires the current head SHA when the field is
  present, excludes the PR author and automation identity, and uses the D3
  `merge.trustedBots` / association policy.
