...
ff-promoted) and caught up to `merge-queue` `0d76ed5` (merge `ac261fc`, PRs
#195, #198) then to `merge-queue` `3edc4bb` (merge `be42cd2`, T4.5 / PR #201,
the `phase-4-done` tag); target `merge-queue`. The version bump to
...
- T4.5 soak/DoD evidence → **landed** in [SELF-HOSTING.md](SELF-HOSTING.md) +
  [docs/dod-evidence.md](docs/dod-evidence.md) (PR #201, `3edc4bb`); the soak
  criterion itself is a recorded **NOT MET** phase-4 deviation whose fix is
  owner action (`secrets.GH_TOKEN`).
...
| 6   | `npm publish --dry-run` + `npm pack` inspection clean; `files` allowlist | met     | [`publish-dry-run.log`](docs/release-evidence/publish-dry-run.log) · [`pack-audit.log`](docs/release-evidence/pack-audit.log) (tarball sha256 `699a72dab4e48bacb5b6c59946da5ef701e88883e2bb2e073c5cfd0087ff71c4`) · [`tarball-listing.txt`](docs/release-evidence/tarball-listing.txt) · CI [pack-audit workflow](.github/workflows/pack-audit.yml) (green on this PR head)                                                                                          |
...
| 9   | Every §10 design debt dispositioned before the tag                       | partial | DD-1 [spike](docs/dd-1-abort-spike.md) · DD-2 [normalization](docs/dd-2-usd-normalization.md) · DD-3/DD-8 [reverify-2026-09](docs/reverify-2026-09.md) · DD-9 [budget](docs/dd-9-api-equivalent-budget.md) · DD-4 fixtures [PR 12](https://github.com/camerontaylor/cq-fixtures/pull/12) · DD-5/DD-6 no v1 action (plan §10) · DD-7 op-result taxonomy in [docs/ops](docs/ops/) — consolidated disposition table [docs/dod-evidence.md](docs/dod-evidence.md) (T4.5) |
...
| 4. Pluggable worker seam: ≥2 working drivers + ADR                                                                                          | WS-B; ADR-0001                          | Four conformance lanes — [src/driver/README.md](src/driver/README.md) · [acp driver](docs/acp-driver.md) · [acp strategy](docs/acp-driver-strategy.md) · live drivers [run 34915473781](https://github.com/camerontaylor/cq-toolkit/actions/runs/34915473781); **ADR-0001** is in the private research repo (`plans/adr/0001-worker-driver-seam.md`) with status line still `proposed` — **ADR portion pending owner** — the four lanes and conformance suite are delivered; the ADR lives in the private research repo and is still `proposed` ([docs/dod-evidence.md](docs/dod-evidence.md) DoD 4)                                                                                                                                                                                                                                                                                                                                                                                       |
| 5. Fixtures/eval repo: scored suites for fixer-worker + review-classifier; per-role comparison tables (outcome/cost/wall time); CI-runnable | WS-J                                    | Real-driver matrix [fixtures PR 13](https://github.com/camerontaylor/cq-fixtures/pull/13) · snapshot tables [PR 16](https://github.com/camerontaylor/cq-fixtures/pull/16) · suite CI green main `6768530` [run 35523233617](https://github.com/camerontaylor/cq-fixtures/actions/runs/35523233617) · J6 release wiring [PR 17](https://github.com/camerontaylor/cq-fixtures/pull/17) · phase-4 exit gate recorded 2 ws-j FAIL rows + 1 DEFERRED→T5.2 (driver-axis lanes errored on CI → honest zeros) — owner list in the release runbook                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| 7. No staging: one coherent v1 release                                                                                                      | plan §1, §7 (release = P4 single event) | This release PR carries the DoD checklist with each evidenceable row linked; it does **not** yet claim all rows complete — npm publication, the soak criterion (recorded NOT MET), ADR acceptance and the tags gate the release · [CHANGELOG.md](CHANGELOG.md) v1.0.0 (merged PR titles by workstream); **MISSING: `v1.0.0` tag refs on toolkit + fixtures (T5.2/T5.3)**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
- round 4 (final-head verification after the high fix): 3 findings (2 medium,
  1 low) — **all fixed**: caught up to `3edc4bb` (T4.5) and repointed the
  soak/DoD rows; added #201 to `CHANGELOG.md`; added the final-head gate row.
  Fresh reviewer `9e8ded4` (paseo
...
- pack audit reproduction (CI `.github/workflows/pack-audit.yml`):
  ...
  `699a72dab4e48bacb5b6c59946da5ef701e88883e2bb2e073c5cfd0087ff71c4`; 273
...
- Flip research-repo status lines: ADR-0001 → accepted; plan §6/spec status;
  plan §11 DoD table.
- **DoD 2 soak**: configure `secrets.GH_TOKEN` on cq-toolkit so the scheduled
  `self-review-loop` / `self-merge-prs` automations process PRs — the soak is
  a recorded **NOT MET** phase-4 deviation ([SELF-HOSTING.md](SELF-HOSTING.md),
  [docs/dod-evidence.md](docs/dod-evidence.md)).
- **DoD 4 ADR**: flip ADR-0001 to accepted in the research repo (still
  `proposed`).
- **Fixtures matrix (ws-j)**: the phase-4 exit gate recorded 2 FAIL rows and 1
  DEFERRED→T5.2; the release runbook carries the owner list (driver-axis lanes
  errored on CI → honest zeros).
